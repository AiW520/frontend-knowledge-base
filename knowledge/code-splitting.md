...
代码分割是将应用代码拆分成多个小块（chunk），按需加载，减少首屏加载时间。jinzhuan-project-web 项目通过 Vite 的 `manualChunks` 配置和路由懒加载实现了代码分割优化。
...
在 jinzhuan-project-web 项目中，代码分割主要通过两种方式实现：
...
- 来源文件：`vite.config.ts`（构建配置）
- 来源文件：`src/router/index.ts`（路由懒加载）
- 来源文件：`dist/assets/`（构建产物结构）
