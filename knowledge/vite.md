...
Vite 是下一代前端构建工具，基于原生 ES 模块（ESM），开发服务器秒级启动，使用 Rollup 进行生产构建。Vue3 项目级训练工程使用 Vite 5 作为构建工具。
...
Vue3 项目级训练工程的 Vite 配置（`vite.config.ts`）包含以下主要配置：
...
- 示例来源：通用训练工程抽象
- 示例来源：通用训练工程抽象（完整 Vite 配置）
- 示例来源：通用训练工程抽象（Vite 版本和脚本）
