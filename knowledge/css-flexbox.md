...
在学籍管理系统的 CSS 文件中，使用了 `display: flex` 实现元素的居中对齐和水平排列：
...
在供应链金融项目的 App.vue 中，虽然没有直接使用 Flexbox，但 Element UI 的 `el-row` 组件底层就是基于 Flexbox 实现的。
...
来源：`学籍管理-frontend/css/style.css` 第 70-75 行
...
来源：`供应链—front/src/App.vue` 第 20-31 行
...
- 来源文件：`学籍管理-frontend/css/style.css` 第 70-75 行
- 来源文件：`供应链—front/src/App.vue` 第 20-31 行
