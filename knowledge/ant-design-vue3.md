...
Ant Design Vue 是 Ant Design 的 Vue 3 实现，提供一套高质量的企业级 UI 组件。jinzhuan-project-web 项目使用 Ant Design Vue 3 构建整个管理后台界面。
...
在 jinzhuan-project-web 项目中，Ant Design Vue 3 用于构建后台管理界面：
...
来源：`jinzhuan-project-web/src/main.ts`
...
- 来源文件：`src/main.ts`（Ant Design Vue 全局配置）
- 来源文件：`src/components/Layout/layout.vue`（a-config-provider 使用）
- 来源文件：`src/hooks/useForm.ts`（Modal.confirm 使用）
- 来源文件：`src/fetch/index.ts`（message 使用）
- 来源文件：`components.json`（shadcn-vue 配置）
- 来源文件：`vite.config.ts`（组件自动导入配置）
